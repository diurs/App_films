...
class Window_search(Toplevel):
    ...
    def connect(self):
        connection = pymysql.connect('localhost','root','тут был пароль','kek')
        ...
        for row in cursor:
        	...
        	year_all.append(str(row[0])) #все годы повтор
        	
        	rating_ball.append(str(row[1]))
        	rating_b_all.append(str(row[1])) # все и повтор тоже без сортировки
